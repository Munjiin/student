                  ...
                  <tbody>
                     <c:forEach var="board" items="${selectPage}">
                        <tr class="odd gradeX">
                           <td>${board.bno}</td>
                           <td><a
                              href="read?bno=${board.bno}&page=${pageMaker.pageDTO.page}">${board.title}</td>
                           <td>${board.name}</td>
                           <td>${board.cnt}</td>
                           <td>${board.regdate}</td>
                  ...
                  <div class="well">
                     <ul class="pagination">
                        ...
                        <c:if test="${pageMaker.prev}">
                           <li><a
                              href="list?page=${pageMaker.start-1}&size=${pageMaker.pageDTO.size}">Prev</a>
                           </li>
                        </c:if>
                        ...
                        <c:if test="${pageMaker.next}">
                           <li><a
                              href="list?page=${pageMaker.end+1}&size=${pageMaker.pageDTO.size}">Next</a>
                           </li>
                        </c:if>
